...
// g -> start to current
// h -> heu val of current to goal state
// heu is calculated using euclidean distance
...
function calculateHeuristic(Data, startNode, endNode){
    ...
    for (let i = 0; i < Data.length; i++) {
        for (let j = 0; j < Data.length; j++) {
           Data[i][j].heuristic = Math.abs(Data[i][j].i-endNode.i) + Math.abs(Data[i][j].j-endNode.j); // g+h
        }
...
function AstarPath(path, frames, nodes, speed){
    ...
    setTimeout(function(){
        ...
        pathStatus.innerHTML = `Optimal path : `+path.length+ `<br/>Node visited after searching `+(nodes)+` nodes.`;
